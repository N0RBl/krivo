...
async function handleListPeers(request, env) {
  ...
  if (!network)
    return jsonResponse({ ok: false, error: "Missing network" }, 400);
...
async function handleRelayForward(request, env) {
  ...
  if (!RELAY_TYPES.has(message.type)) {
    return jsonResponse(
      { ok: false, error: "Unsupported relay message type" },
      400,
    );
  }
...
async function registerWithHub(env, selfUrl) {
  const resp = await fetch(
    `${relayHttpBase(normalizeRelayUrl(env.GLOBAL_RELAY_URL))}/api/v1/relays`,
    {
      method: "POST",
    ...
    if (type === "announce") {
      ...
      try {
        sendPeerList(
          socket,
          network,
          mergeDiscoveredPeers(localPeers, remotePeers).filter(
      ...
      if (live) {
        try {
          ...
          deliveredLive = true;
          console.log(
            `[RELAY] Delivered ${type} from ${peerId} to ${message.to} immediately`,
          );
        } catch (err) {
          console.error(
      ...
      if (!deliveredLive && db) {
        ...
        if (!live) {
          console.log(
            `[RELAY] Peer ${message.to} offline, queued ${type} in DB`,
          );
        } else {
          console.log(
            `[RELAY] Queued ${type} for ${message.to} after live delivery failure`,
          );
        }
      ...
      if (!deliveredLive && env.RELAY_URL && env.DB) {
        ctx.waitUntil(
          (async () => {
            ...
            console.log(
              `[FED] Forwarding ${type} to ${remoteUrls.length} peer relay(s) for ${message.to}`,
            );
            await Promise.all(
              remoteUrls.map((u) => forwardToRelay(u, message, selfRelayId)),
            );
          })(),
        );
      }
